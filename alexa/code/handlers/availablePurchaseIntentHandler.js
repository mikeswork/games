...
const AvailablePurchaseIntentHandler = {
    canHandle(handlerInput) {
        return handlerInput.requestEnvelope.request.type === 'IntentRequest'
            && handlerInput.requestEnvelope.request.intent.name === 'askAvailablePurchase';
    },
    async handle(handlerInput) {
        console.log('[AvailablePurchaseIntentHandler]');
        ...
        if (product) {
            if (isp.isProductPurchased(handlerInput)) {
                speechText = `You already own ${product.name}, which gives you ${product.summary}. `;
...
module.exports = AvailablePurchaseIntentHandler;